...
exports.WAConnection = _WAConnection => {
  class WAConnection extends _WAConnection {
   ...
   sendText(jid, text, options = {}) {
    return this.sendMessage(jid, text, 'conversation', options)
    }
   async getMe() {
...
`.trim()
   ...
   async onWhatsApp(jiid) {
   let { status, biz, jid } = await this.query({ json: ['query', 'exist', jiid], requiresPhoneConnection: false });
  let isBusiness = biz ? true : false
   if (status == 200) return { jid, exists: true, isBusiness }
   return { exists: false, isBusiness }
   }
     async sendContactArray(jid, kon, opts = {}) {
     let list = []
     for (let i of kon) {
     var statusnya = (await this.getStatus(i)).status
  list.push({ "displayName": this.getName(i), "vcard": `BEGIN:VCARD\nVERSION:3.0\nORG:${statusnya == 401 ? '' : statusnya}\nFN:${this.getName(i)}\nitem1.TEL;waid=${i.split('@')[0]}:${i.split('@')[0]}\nitem1.X-ABLabel:Contact\nEND:VCARD` })
  }
 ...
 let grup = []
  ...
  return participants.filter(a => a.isAdmin)
    ...
    return require('./functions').parseResult(json, options)
  }
    /**
     * Fake Replies
     * @param {String} jid 
...
exports.smsg = (conn, m, hasParent) => {
  ...
  if (m.key) {
    ...
    m.id = m.key.id
    m.isBaileys = m.id.startsWith('CLPH') || m.id.startsWith('3EB0') && m.id.length === 12
 ...
 //  db_user = require('../database').users[m.sender]
  ...
  if (m.message) {
    ...
    if (m.quoted) {
      ...
      m.quoted.isBaileys = m.quoted.id ? m.quoted.id.startsWith('CLPH') || m.quoted.id.startsWith('3EB0') && m.quoted.id.length === 12 : false
